...
class DefaultType : public ArgVisitor<DefaultType, char> {
  public:
	...
	template <typename T> char visit_any_int(T) { return 'd'; }

	template <typename T> char visit_any_double(T) { return 'g'; }
...
class BasicPrintfArgFormatter : public internal::ArgFormatterBase<Impl, Char, Spec> {
  ...
  public:
	 ...
	 */
	BasicPrintfArgFormatter(BasicWriter<Char> &w, Spec &s) : internal::ArgFormatterBase<Impl, Char, Spec>(w, s) {}
...
inline void printf(Writer &w, CStringRef format, ArgList args) { PrintfFormatter<char>(args, w).format(format); }
FMT_VARIADIC(void, printf, Writer &, CStringRef)

inline void printf(WWriter &w, WCStringRef format, ArgList args) { PrintfFormatter<wchar_t>(args, w).format(format); }
