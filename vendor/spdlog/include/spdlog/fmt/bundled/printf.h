...
public:
    ...
    printf_arg_formatter(internal::basic_buffer<char_type> &buffer, format_specs &spec, context_type &ctx)
        : base(back_insert_range<internal::basic_buffer<char_type>>(buffer), &spec, ctx.locale())
        , context_(ctx)
    {
    }
...
unsigned basic_printf_context<OutputIt, Char, AF>::parse_header(iterator &it, format_specs &spec)
{
    ...
    {
        ++it;
        spec.width_ = visit_format_arg(internal::printf_width_handler<char_type>(spec), get_arg(it));
    }
    return arg_index;
}
...
void basic_printf_context<OutputIt, Char, AF>::format()
{
    ...
    while (*it)
    {
        ...
        if (*it == '.')
        {
            ...
            {
                ++it;
                spec.precision = visit_format_arg(internal::printf_precision_handler(), get_arg(it));
            }
            else
            {
                spec.precision = 0;
...
inline std::basic_string<Char> vsprintf(
    const S &format, basic_format_args<typename basic_printf_context_t<internal::basic_buffer<Char>>::type> args)
{
...
inline FMT_ENABLE_IF_T(internal::is_string<S>::value, std::basic_string<FMT_CHAR(S)>) sprintf(const S &format, const Args &... args)
{
...
template<typename S, typename Char = FMT_CHAR(S)>
inline int vfprintf(
    std::FILE *f, const S &format, basic_format_args<typename basic_printf_context_t<internal::basic_buffer<Char>>::type> args)
{
    basic_memory_buffer<Char> buffer;
    printf(buffer, to_string_view(format), args);
 ...
 */
template<typename S, typename... Args>
inline FMT_ENABLE_IF_T(internal::is_string<S>::value, int) fprintf(std::FILE *f, const S &format, const Args &... args)
{
    internal::check_format_string<Args...>(format);
    typedef internal::basic_buffer<FMT_CHAR(S)> buffer;
    typedef typename basic_printf_context_t<buffer>::type context;
...
template<typename S, typename Char = FMT_CHAR(S)>
inline int vprintf(const S &format, basic_format_args<typename basic_printf_context_t<internal::basic_buffer<Char>>::type> args)
{
 ...
 */
template<typename S, typename... Args>
inline FMT_ENABLE_IF_T(internal::is_string<S>::value, int) printf(const S &format_str, const Args &... args)
{
...
inline FMT_ENABLE_IF_T(internal::is_string<S>::value, int)
    fprintf(std::basic_ostream<FMT_CHAR(S)> &os, const S &format_str, const Args &... args)
{
    internal::check_format_string<Args...>(format_str);
    typedef internal::basic_buffer<FMT_CHAR(S)> buffer;
    typedef typename basic_printf_context_t<buffer>::type context;
    format_arg_store<context, Args...> as{args...};
    return vfprintf(os, to_string_view(format_str), basic_format_args<context>(as));
}
FMT_END_NAMESPACE
