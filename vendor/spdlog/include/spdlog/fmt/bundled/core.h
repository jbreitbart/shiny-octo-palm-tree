...
#if FMT_HAS_FEATURE(cxx_explicit_conversions) || FMT_GCC_VERSION >= 405 || FMT_MSC_VER >= 1800
...
public:
    ...
    FMT_CONSTEXPR basic_string_view(const std::basic_string<Char, Alloc> &s) FMT_NOEXCEPT : data_(s.data()), size_(s.size())
    {
    }

#ifdef FMT_STRING_VIEW
    FMT_CONSTEXPR basic_string_view(FMT_STRING_VIEW<Char> s) FMT_NOEXCEPT : data_(s.data()), size_(s.size()) {}
#endif
...
struct formatter
{
    static_assert(internal::no_formatter_error<T>::value, "don't know how to format the type, include fmt/ostream.h if it provides "
...
template<typename S>
struct is_string : std::integral_constant<bool, !std::is_same<dummy_string_view, decltype(to_string_view(declval<S>()))>::value>
{
};
...
FMT_CONSTEXPR typename std::enable_if<std::is_same<typename C::char_type, Char>::value, init<C, int, char_type>>::type make_value(Char val)
{
    return val;
}
...
template<typename C, typename T>
inline typename std::enable_if<std::is_enum<T>::value && convert_to_int<T, typename C::char_type>::value, init<C, int, int_type>>::type
...
template<typename C, typename T, typename Char = typename C::char_type>
inline typename std::enable_if<!convert_to_int<T, Char>::value && !std::is_same<T, Char>::value &&
...
FMT_CONSTEXPR11 typename std::enable_if<internal::is_string<S>::value, init<C, basic_string_view<typename C::char_type>, string_type>>::type
make_value(const S &val)
{
    // Handle adapted strings.
    static_assert(std::is_same<typename C::char_type, typename internal::char_t<S>::type>::value,
...
private:
    ...
    friend FMT_CONSTEXPR typename internal::result_of<Visitor(int)>::type visit_format_arg(Visitor &&vis, const basic_format_arg<Ctx> &arg);
...
FMT_CONSTEXPR typename internal::result_of<Visitor(int)>::type visit_format_arg(Visitor &&vis, const basic_format_arg<Context> &arg)
{
    typedef typename Context::char_type char_type;
    switch (arg.type_)
    {
...
// DEPRECATED!
template<typename Visitor, typename Context>
FMT_CONSTEXPR typename internal::result_of<Visitor(int)>::type visit(Visitor &&vis, const basic_format_arg<Context> &arg)
{
...
private:
    ...
    {
        return IS_PACKED ? internal::get_types<Context, Args...>() : internal::is_unpacked_bit | NUM_ARGS;
    }
...
typename buffer_context<Char>::type::iterator vformat_to(
    internal::basic_buffer<Char> &buf, basic_string_view<Char> format_str, basic_format_args<typename buffer_context<Char>::type> args);
} // namespace internal
...
inline typename std::enable_if<is_contiguous<Container>::value && internal::is_string<S>::value, std::back_insert_iterator<Container>>::type
